...
## التشغيل السريع (الواجهة الموحّدة)
...
| 4 | كود حقيقي لا سطحي | ⚠️ بوابات صدق تمنع الوهمي؛ المحتوى الحقيقي يحتاج عقلاً | `agent_os.py` |
...
| 7 | فكر مالي + تقني | ◻️ دفاتر مالية قائمة (لم تُطوَّر هذا السيشن) | `agent_os/revenue_engine.py` |
...
| 11 | أولويات من محادثاتنا | ⚠️ جزئي (استدعاء الذاكرة + الاستراتيجية) | `run_task._recall_memory` |
...
## الاختبارات
```bash
python -m pytest tests/ -q      # 292 ناجح (وقت كتابة هذه الوثيقة)
```
